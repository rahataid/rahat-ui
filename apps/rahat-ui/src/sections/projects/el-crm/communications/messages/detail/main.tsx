...
import { ArrowLeft, RefreshCcw, Send, Info } from 'lucide-react';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
          ...
          <div className="flex gap-2">
            {(count && count?.FAIL > 0) ||
              (count?.SCHEDULED > 0 && (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button type="button" onClick={retryFailed} size="sm" variant="outline" className="gap-2">
                      <RefreshCcw className="h-3.5 w-3.5" />
                      Retry Failed
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>Retry all failed message deliveries</TooltipContent>
                </Tooltip>
              ))}
            <Tooltip>
              <TooltipTrigger asChild>
              ...
              <TooltipContent>
                {campaign.sessionId ? 'Message already sent' : 'Send this message now'}
              </TooltipContent>
            </Tooltip>
          </div>
        </div>
