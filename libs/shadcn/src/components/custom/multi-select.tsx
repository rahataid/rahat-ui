...
interface MultipleSelectorProps {
  ...
  /** Loading state - shows spinner when options are being fetched */
  loading?: boolean;

  /** Loading message to display when loading is true */
  ...
  /** Custom className for CommandGroup component */
...
const MultipleSelector = ({
  ...
  // Handler for input value changes - used both for user input and programmatic clearing
    ...
    const Item = (
      <CommandItem
        ...
        onSelect={(value: string) => {
          ...
          // Clear input/search text - use the same handler to ensure cmdk updates
  ...
  const commandFilter = React.useCallback(() => {
    ...
    // Custom filter that searches by label (displayed text) instead of value
    return (value: string, search: string) => {
      if (!search) return 1;
      ...
      const option = allOptions.find((opt: Option) => opt.value === value);
      if (!option) return -1;

      // Search in the label (displayed text)
      const labelLower = option.label.toLowerCase();
      if (labelLower.includes(searchLower)) {
        return 1;
      }

      // Also search in value as fallback
  ...
  }, [commandProps?.filter, creatable, allOptions]);
  ...
  return (
    ...
    >
      <div
        className={cn(
          'border-input focus-within:border-ring rounded focus-within:ring-ring/50 has-aria-invalid:ring-destructive/20 dark:has-aria-invalid:ring-destructive/40 has-aria-invalid:border-destructive relative min-h-[38px] border text-sm transition-[color,box-shadow] outline-none focus-within:ring-[3px] has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50',
          {
          ...
          {selected.map((option) => {
            return (
              <div
                key={option.value}
                className={cn(
                  'animate-fadeIn bg-background rounded text-secondary-foreground hover:bg-background relative inline-flex h-7 cursor-default items-center border pr-7 pl-2 text-xs font-medium transition-all disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 data-fixed:pr-2',
                  badgeClassName,
                )}
      ...
      <div className="relative">
        <div
          ...
          style={{
            // Ensure stable positioning - fixed distance from top
            top: 'calc(100% + 0.5rem)',
            left: 0,
            right: 0,
          }}
            ...
            >
              {isLoading || loading ? (
                <div className="flex items-center justify-center p-6">
                  {loadingIndicator || <p>{loadingMessage}</p>}
                </div>
              ) : (
                <>
                  {EmptyItem()}
