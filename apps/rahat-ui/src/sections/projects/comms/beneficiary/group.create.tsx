  ...
  const handleCreateGroup = async (data: z.infer<typeof FormSchema>) => {
    try {
      ...
      if (result) {
        ...
        router.push(`/projects/comms/${id}/beneficiary?tab=beneficiaryGroups`);
      }
    } catch (e: any) {
      toast.error(
      ...
      <form onSubmit={form.handleSubmit(handleCreateGroup)}>
        <div className="p-4 h-[calc(100vh-115px)]">
          <HeaderWithBack
            ...
            path={`/projects/comms/${id}/beneficiary?tab=beneficiaryGroups`}
          />
        ...
        <div className="flex justify-end space-x-2 py-2 px-4 border-t">
          <Button
            ...
            onClick={() =>
              router.push(
                `/projects/comms/${id}/beneficiary?tab=beneficiaryGroups`,
              )
            }
          >
            Cancel
